...
- 그 중 **대표 repo 1~5개** 를 자동 선정해 카드 형태로 표시 (사이드바 슬라이더로 개수 조절)
...
| 유형 | 확인 포인트 |
| --- | --- |
| repo가 많은 사용자 (예: 유명 오픈소스 메인테이너) | shallow 페이지네이션 / 대표 repo 1~5 선정 / 캐시 동작 |
...
- 앱이 정상 실행되는가
- 사이드바에서 지정한 개수(1~5) 만큼 대표 repo 카드와 PDF 대표 저장소가 표시되는가
