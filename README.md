...
- **게스트 모드** — 비로그인 상태에서도 임의 username 의 공개 repo 를 가볍게 체험 분석 가능 (로그인 없이 첫인상 확인). 게스트가 사이드바에 본인 GitHub PAT 를 입력하면 인증 호출로 GitHub rate limit 을 완화할 수 있음
...
- 기술 스택 추출 — 언어(byte 비중)는 전부 노출하고, 설정 파일·디렉토리·DB 드라이버·`.env` 예시 변수명 기반으로 프레임워크/DB 엔진/도구를 추론해 범주별(언어·프론트엔드·백엔드·데이터ML·데이터베이스·모바일·테스트·인프라)로 묶어 표시
...
- LLM 생성 문장(요약 / 포트폴리오 문장 / 이력서 bullet)에 **복사 버튼** 제공, "AI 초안 · 검토 필요" 안내는 리포트 상단 1곳으로 통합 (제출 전 외부 문서에 붙여넣어 다듬는 흐름 지원. 예상 면접 질문은 복사 버튼 없이 표시)
...
모드는 별도 토글이 아니라 **로그인 여부로 자동 결정**됩니다. 같은 분석 파이프라인을 쓰지만, 인증 수단에 따라 분석 범위·호출 한도·정확도 보강이 달라집니다.
...
| GitHub 인증 | 미인증 호출 | 사용자 PAT 로 인증 호출 | OAuth access token 으로 인증 호출 |
| GitHub rate limit | 시간당 60회 | 시간당 5,000회 | 시간당 5,000회 |
| 대표 repo 최대 개수 | 3 | 5 | 5 |
| README 스니펫 peek (선정 정확도↑) | 비활성 | 활성 | 활성 |
| 토큰 취급 | — | 메모리에만 보관, 캐시 키엔 fingerprint(해시 앞부분)만 포함 | 서버 라우트에서 `getToken()` 으로만 접근, 클라이언트 비노출 |

보충 설명:

- **README 스니펫 peek**: 대표 repo 선정 시 후보의 README 앞부분(~500자)을 LLM 에 함께 보내 정확도를 높이는 기능입니다. 추가 GitHub 호출이 들기 때문에 **인증 호출일 때만** 켭니다. 따라서 "게스트(토큰 없음)"이라도 서버에 `GITHUB_TOKEN` 이 설정돼 있으면 그 토큰으로 인증 호출이 되어 rate limit 5,000회 + peek 활성으로 동작합니다. (서버 토큰도 없을 때만 60회/미활성)
- **게스트 PAT**: 새로고침하면 사라지는 메모리 보관값이며, 응답·로그·캐시 키 어디에도 원문 토큰이 들어가지 않습니다. PAT 는 호출 한도 완화 용도이고, public 모드에서는 어차피 공개 repo 만 조회합니다.
- **로그인(self) 모드**: 서버가 OAuth 토큰의 실제 사용자명과 요청 username 을 대조해 "남의 계정 분석"을 차단합니다.
...
사이드바 분석 시작
  └─► POST /api/analyze
        ...
        │     └─ README / 루트 트리 + 재귀 파일 경로(정제) / languages / 최근 commit 10 / 설정 파일 / .env 예시 변수명
        ├─ 규칙 기반 분석
        │     ├─ README 신뢰도 (readmeReliability.ts)
        │     ├─ 기술 스택 추출 (techStack.ts → 언어 byte 비중 분리 + 프레임워크/DB 엔진/도구 추론)
        ...
        │     ├─ 사용자 리포트 1회
        │     └─ repo 분석 묶음 1회 (N개 모아서 단일 호출, 재귀 파일 경로·env 변수명으로 핵심 기술 자산 우선 식별)
        │     └─ temp=0 + seed + stableStringify 로 호출 재현성 확보
        ├─ 단계마다 onProgress 콜백 → NDJSON progress 이벤트로 스트리밍 (route.ts)
        └─ 최종 AnalyzeResponse 를 done 이벤트로 전송 (캐시 hit 은 스트리밍 없이 즉시 JSON)
  └─► Dashboard 렌더링
...
**1. 언어 (repo 메타에서 바로 나오는 1차 정보 → 전부 노출)**

GitHub languages API 의 **byte 비중**을 그대로 사용하며, 임계값 없이 들어간 언어를 전부 비중(%)과 함께 표시합니다. (예: `TypeScript 84% · CSS 9% · PLpgSQL 4%`) byte 분포를 못 가져오면 주 언어만 100% 로 폴백합니다.

**2. 추론한 기술 스택 (코드·설정 기반으로 알아내는 부분)**

여러 신호에 가중치를 매겨 누적한 뒤, **합산 weight ≥ 2 인 항목 상위 12개**만 채택합니다. 표시할 때는 순수 언어 항목을 빼고(위 1번과 중복 방지) 범주별로 묶습니다.
...
| 언어 byte 비중 | ≥50% → 3, ≥10% → 2, 그 미만 → 1 (잡음 언어 자동 탈락) | 1~3 |
...
commit author timestamp 를 사용자가 고른 타임존으로 환산해 계산합니다 (`activityPattern.ts`, 기본 `Asia/Seoul`, Intl 기반 DST 반영). 표본이 부족하면 `commit 표본 부족` 만 답니다.

- **시간대 구간**: 야간 21~03시 / 오전 05~11시 / 오후·저녁 12~20시
- **일관성(consistency)**: commit 이 특정 기간에 몰리지 않고 여러 날짜에 고르게 분포하는지를 0~1 로 나타낸 지표 (표본 수로 보정)
...
| 야간 활동 경향 | 야간 비율 ≥ 0.4 |
| 오전 활동 경향 | 오전 비율 ≥ 0.4 |
| 오후/저녁 활동 경향 | 오후 비율 ≥ 0.4 |
| 시간대 고르게 분포 | 위 세 구간 중 두드러진 곳이 없을 때 |
| 주말 집중형 | 주말 비율 ≥ 0.35 |
| 주중 집중형 | 주말 비율 ≤ 0.1 |
| 꾸준한 커밋형 | 일관성 ≥ 0.5 |
| 단기 몰입형 | 0 < 일관성 < 0.2 |
| 최근 활동 활발 | 최근 활동(1년 이내) 있음 |
| commit 표본 부족 | 표본이 통계 판단에 부족할 때 |
...
- 활동 패턴은 사용자가 선택한 타임존(기본값은 브라우저 감지 타임존)으로 환산해 계산합니다. 타임존을 잘못 선택하면 야간/오전/오후/주말 분류가 실제와 달라질 수 있습니다.
...
- 기술 스택 분석은 분석 대상의 `.env.example` 류에서 **변수 이름만** 참고합니다(실제 `.env` 는 읽지 않음). 따라서 예시 파일이 없거나 변수명이 일반적(`API_KEY` 등)이면 외부 연동을 충분히 추정하지 못할 수 있습니다.

## 향후 개선 방향

### 게스트(다른 username) 모드 운영 방침

현재는 모드가 로그인 여부로 자동 결정됩니다. 로그인 상태면 항상 self(본인 계정), 로그아웃 상태면 public(임의 username 의 공개 저장소)으로 동작합니다. 즉 "게스트 모드"는 별도 토글이 아니라 **로그아웃 상태 자체**입니다.

게스트 모드는 **로그인 없이 가볍게 체험**할 수 있는 진입점이라 기본적으로 유지합니다. 다만 미인증 호출은 GitHub rate limit(60회/시간)과 임의 username 분석이라는 특성이 있으므로, 만약 "본인이 본인 저장소만 분석" 시나리오로 좁히고 싶다면 `app/api/analyze/route.ts` 의 미로그인(public) 분기와 사이드바의 username·게스트 PAT 입력 섹션을 제거해 로그인 필수로 전환할 수 있습니다. (선택 사항)
...
- 핵심 소스 파일 본문 스니펫까지 LLM 입력에 추가 (현재는 재귀 파일 경로·설정·env 변수명까지만 사용. 토큰 비용 대비 효과를 보고 결정)
...
- **분석 대상 시크릿 비노출(현재 적용)**: 기술 스택 분석 시 분석 대상 repo 의 실제 `.env`·`.env.local` 등은 **읽지 않고** `*.example`·`*.sample`·`*.template` 만 대상으로 합니다. 그마저도 **변수 이름만** 추출하고 값(placeholder 포함)은 전부 버려, 분석 대상의 시크릿이 LLM 입력이나 결과·로그에 들어가지 않습니다.
...
- README 신뢰도 등급
- 기술 스택 후보 + 신뢰도 가중치 (언어 비중과 분리)
- 정제한 재귀 파일 경로(예: `lib/llm.ts`) + `.env` 예시 변수명(예: `OPENAI_API_KEY`, `DATABASE_URL`) → 의존성만으로는 안 잡히는 **핵심 기술 자산/외부 연동 신호**
- 분야별 점수 (0~100)
- 활동 패턴 (시간/요일/일관성)
- 추론 메모 (예: "설정 파일 기준 프론트엔드 웹앱으로 추정", "환경 변수 예시 기준 LLM API 연동 추정")

그 다음 이 정리된 결과(JSON)를 LLM에 전달해 자연어 리포트, 포트폴리오 문장, 이력서 bullet, 예상 면접 질문을 생성합니다. 이때 프롬프트는 설정 도구(ESLint/Husky 등)보다 **핵심 기술 자산(외부 연동·아키텍처)을 우선 식별**하도록 지시합니다. 즉, **Gen AI는 "데이터 수집" 이 아니라 "해석된 결과를 커리어 언어로 변환하는 단계"** 에 사용됩니다.
