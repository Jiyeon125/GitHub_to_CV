...
- LLM 생성 문장(요약 / 포트폴리오 / 이력서 bullet / 면접 질문)에 **복사 버튼 + "AI 초안 · 검토 필요" 안내** 제공 (제출 전 사용자가 외부 문서에 붙여넣어 다듬는 흐름 지원)
...
- LLM 생성 결과는 자동 보정된 사실이 아니라 추정이며, 제출 전 반드시 사용자가 검토해야 합니다. (UI 의 복사 버튼으로 문장을 가져가 외부 문서에서 다듬는 흐름을 권장합니다.)
