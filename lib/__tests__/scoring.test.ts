...
describe("computeShallowScore", () => {
  it("최근 + description 만 있는 기본 repo 는 recency(25)+description(15)=40", () => {
    const { score, breakdown } = computeShallowScore(makeRepo());
    expect(score).toBe(40);
    expect(breakdown.recencyWeighted).toBe(25);
    expect(breakdown.descriptionWeighted).toBe(15);
...
describe("refineScoreWithDeepData", () => {
  ...
  it("README + 충분한 구조(엔트리>=3)면 readme15/structure20 가산", () => {
    ...
    expect(refined.score).toBe(75); // 25 + 15 + 15 + 20
  });
...
describe("rankRepresentativeRepos", () => {
  it("점수 내림차순으로 정렬하고 limit 만큼 자른다", () => {
    const repos = [
      makeRepo({ id: 1, name: "a", topics: ["x"] }), // 45
      makeRepo({ id: 2, name: "b", fork: true, description: null }), // 15
      makeRepo({ id: 3, name: "c" }), // 40
    ];
